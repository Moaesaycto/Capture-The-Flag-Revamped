...
const ChatTabs = ({ chats, openChat, setOpenChat }: ChatTabsProps) => {
  ...
  return (
    <div className="bg-neutral-900 h-12 w-full flex flex-row items-center justify-around text-white text-lg p-2 gap-2 ">
      {chats.map((c, k) => (
        <li
          key={k}
          className={`list-none flex items-center gap-2 h-full w-1/2 px-2 justify-center hover:cursor-pointer rounded
          ${openChat == c.type ? "bg-neutral-950 shadow-xl" : "hover:bg-neutral-700"}`}
